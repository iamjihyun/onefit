...
<body>
	...
	<c:if test="${ loginUser.status  eq 3 }">
		<c:import url="user/pay.do" />
		<a href="user/pay.do"></a>
	</c:if>
	...
	<div class="outer">
		...
		<div class="Div1" align="center">
			...
			<div class="proImgDiv" align="center">
				<img src="resource/profileImage/${ loginUser.chName }" width="250"
					height="250">
			</div>
			...
			<div class="myInfoDiv">
				...
				<table class="myInfoTable table table-sm">
					...
					<tr>
						<th>회원등급 :</th>
						<td><span class="membership">일반회원</span></td>
					</tr>
					<tr>
						<th>생년월일 :</th>
			...
			<div class="exerciseDiv">
				...
				<table class="myInfoTable table table-sm" align="left">
					<tr>
						<th>남은이용기간 :</th>
						<c:if test="${ !empty loginUser.startDate }">

												
							<td><fmt:formatDate value="${ loginUser.endDate }"
									pattern="yyyy-MM-dd" /></td>
						</c:if>
					...
					<tr>
						<th>헬스장 이용기간 :</th>
						<td><fmt:formatDate value="${ loginUser.startDate }"
									pattern="yyyy-MM-dd" />~<fmt:formatDate value="${ loginUser.endDate }"
									pattern="yyyy-MM-dd" /></td>
					</tr>
			...
			<div class="modal-content health_modal_content">
				...
				<div class="modal-body health_modal_content">
					<div class="modal_health_div">
						<form id="healthForm" action="bodyUpdate.do" method="post"
							onsubmit="return validate();" enctype="multipart/form-data">
							<input type="hidden" name="userNum"
								value="${ loginUser.userNum }">
							<table align="center" class="table table-sm modalHealthTable">
								<tr>
									<th>키</th>
									<td><input type="text" class="height" name="height"
										id="height" placeholder="${ loginUser.height }" required>cm</td>
								</tr>
								<tr>
									<th>몸무게</th>
									<td><input type="text" class="weight" name="weight"
										id="weight" placeholder="${ loginUser.weight }" required>kg</td>
								</tr>
							</table>
						</form>
					</div>
